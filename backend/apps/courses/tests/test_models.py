...
class CourseImageUrlTest(BaseTestCase):
    ...
    def test_image_path_generation_with_jpg(self, mock_url):
        ...
        self.assertRegex(path, "^courses/course_")
        uuid_pattern = (
            "^courses/course_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\\.jpg$"
        )
...
class SectionSaveTest(BaseTestCase):
    ...
    def test_auto_increment_when_section_id_not_set(self):
        section_1 = Section.objects.create(course=self.course, title="Section 1")
        section_2 = Section.objects.create(course=self.course, title="Section 2")
...
class CourseEnrollmentIsActiveTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.course = create_test_course()
        self.payment = Payment.objects.create(user=self.user, total_sum=1000, status="success")
...
class CoursePriceValidationTest(BaseTestCase):
    ...
    def test_overflow_price_fails_validation(self):
        with self.assertRaises(OverflowError):
            create_test_course(price=99999999999999999999999999)
...
class PrepareFilesForVsTest(BaseTestCase):
    ...
    def _overview_name(self):
        return f"course-{self.course.pk}-overview.txt"

    def _lesson_name(self, s, l):
        return f"course-{self.course.pk}-s{s}-l{l}.txt"
    ...
    def test_course_without_sections_returns_only_overview(self):
        ...
        self.assertEqual(filename, self._overview_name())
    ...
    def test_course_with_section_and_lesson_returns_lesson_file(self):
        ...
        self.assertIn(self._overview_name(), filenames)
        self.assertIn(self._lesson_name(1, 1), filenames)
    ...
    def test_lesson_file_contains_context_header(self):
        ...
        files = self.course.prepare_files_for_vs()
        lesson_file = next(c for f, c in files if f == self._lesson_name(1, 1))
    ...
    def test_lesson_file_filename_reflects_section_and_lesson_numbers(self):
        ...
        filenames = {f for f, _ in files}
        self.assertIn(self._lesson_name(1, 1), filenames)
        self.assertIn(self._lesson_name(1, 2), filenames)
    ...
    def test_multiple_sections_produce_correct_filenames(self):
        ...
        filenames = {f for f, _ in files}
        self.assertIn(self._lesson_name(1, 1), filenames)
        self.assertIn(self._lesson_name(2, 1), filenames)
