...
class DecoratorIntegrationTestMixin:
    ...
    def assert_decorator_blocks_access(self, decorator, user, expected_status=status.HTTP_403_FORBIDDEN, **kwargs):
        test_view = create_test_view_with_decorator(decorator)
        request = create_authenticated_request(self.factory, user=user)
        response = test_view(request, **kwargs)
...
class RequireCourseAuthorIntegrationTest(DecoratorIntegrationTestMixin, BaseTestCase):
    ...
    def test_moderator_can_access_any_course(self):
        self.assert_decorator_allows_access(require_course_author, self.moderator, course_id=self.course.course_id)

    def test_course_author_can_access(self):
        self.assert_decorator_allows_access(require_course_author, self.teacher, course_id=self.course.course_id)

    def test_non_author_teacher_cannot_access(self):
        self.assert_decorator_blocks_access(require_course_author, self.other_teacher, course_id=self.course.course_id)

    def test_student_cannot_access(self):
        self.assert_decorator_blocks_access(require_course_author, self.student, course_id=self.course.course_id)
...
class RequireCourseEnrollmentIntegrationTest(DecoratorIntegrationTestMixin, BaseTestCase):
    ...
    def test_moderator_can_access_any_course(self):
        self.assert_decorator_allows_access(require_course_enrollment, self.moderator, course_slug=self.course.slug)

    def test_course_author_can_access(self):
        self.assert_decorator_allows_access(require_course_enrollment, self.teacher, course_slug=self.course.slug)

    def test_enrolled_student_can_access(self):
        self.assert_decorator_allows_access(require_course_enrollment, self.student, course_slug=self.course.slug)

    def test_non_enrolled_student_cannot_access(self):
        self.assert_decorator_blocks_access(require_course_enrollment, self.other_student, course_slug=self.course.slug)
    ...
    def test_expired_enrollment_blocks_access(self):
        ...
        self.assert_decorator_blocks_access(require_course_enrollment, self.other_student, course_slug=expired_course.slug)
